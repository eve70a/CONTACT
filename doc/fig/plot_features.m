...
for ift = 1 : slcs.nfeat
   ip = slcs.iseg_p(ift);
   plot3( slcs.u(islc), slcs.ysurf(islc,ip), slcs.zsurf(islc,ip), '.:', 'color',matlab_color(4), ...
                                                               'linewidth',1, 'markersize',9 );
end
clear x y z is islc ift ip;

plot_arrow([92,30,0], [15,0,0], 4);
text(103, 34, 0, '$u_{fc}$ (track curve)', 'interpreter','latex', 'fontsize',14);
...
if (print_fig)
   % adjust towards earlier fontsize in printed picture
   p = get(gcf,'position'); p(3:4) = 1.25*[560 420]; set(gcf,'position',p);
