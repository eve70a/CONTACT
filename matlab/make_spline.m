...
% penalty on 2nd or 3rd derivative
   ...
   %  - advanced algorithm: no more than 1 knot per meas.segment, no smaller than ds_bspl, 
   %    fewer at kinks, accel and boundaries
...
%  - keep_reduc    = flag indicating that tj is/isnt kept in reduced knots t^*
   ...
   % eliminate round-off: collapse tj(j) --> tj(j+1) where |dt|<tiny_dt

   for j = nknot-1 : -1 : 1
      if (abs(tj(j+1)-tj(j))<tiny_dt)
         tj(j) = tj(j+1);
      end
   end
   ...
   if (make_plot)
      ksi  = unique_knots(tj, tiny_dt);
      nksi = length(ksi);
      mult_j = zeros(nksi,1);
      mult_r = zeros(nksi,1);
      for iksi = 1 : nksi
         mult_j(iksi) = nnz(abs(tj-ksi(iksi))<tiny_dt);
         mult_r(iksi) = nnz(abs(tj-ksi(iksi))<tiny_dt & keep_reduc);
      end
...
% simple version that just applies ds_bspl as much as possible -- needs lambda>0 to avoid singular systems
   ...
   naccel = length(iaccel);
   if (idebug>=3)
      disp(sprintf('knot_vector_simple: nmeas=%d, nkink=%d, naccel=%d',nmeas,nkink,naccel))
   end
   ...
   if (idebug>=3)
      disp(['knot_vector_simple: iranges=',num2str(iranges)])
   end
   ...
   for isec = 1 : nsec
      ...
      % Note: this may introduce round-off differences between replicated knots
   
      tj   = [tj; si(sec(1)) + [0:nintv]' * dt ];
   
      if (idebug>=3)
         disp(sprintf('section %d of %d: len=%5.1f, dt=%5.3f, nintv=%3d, adding knots %7.3f : dt : %7.3f', ...
                   isec, nsec, len_sec, dt, nintv, si(sec(1)), tj(end)));
      end
...
function [ ksi ] = unique_knots( tj, tiny_dt )

% function [ ksi ] = unique_knots( tj, tiny_dt )
%
% determine break points ksi from knot sequence tj

   if (nargin<2 | isempty(tiny))
      tiny_dt = 1e-12;
   end
   ...
   if (0==1)
      ...
      dtj   = [1; diff(tj)];
      iuniq = find(abs(dtj)>tiny_dt);
      ksi   = tj(iuniq);

      % mask  = zeros(size(tj));
      % mask(iuniq) = 1;
      % disp([ tj'; dtj'; abs(dtj')>tiny; mask' ])
      % disp(sprintf('knot vector tj: %d knots, %d break points', length(tj), length(ksi)));
   end

end % function unique_knots
