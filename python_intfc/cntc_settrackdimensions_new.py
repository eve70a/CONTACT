...
def cntc_settrackdimensions_new(ire, ztrack, params):
    ...
    if (not isinstance(params, np.ndarray)):
        if (isinstance(params, list)):
            ...
            sys.exit('ERROR in cntc_settrackdimensions_new: invalid params provided.')

    nparam = len(params)

    cntc_dll.cntc_settrackdimensions_new(c_int(ire), c_int(ztrack), c_int(nparam), 
                                                                 params.ctypes.data_as(POINTER(c_double)))

# end function cntc_settrackdimensions_new
